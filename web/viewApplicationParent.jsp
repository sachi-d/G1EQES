<%-- 
    Document   : viewApplicationParent
        ...
        <div class="wrapper">
            ...
            <div class="content-wrapper">
                ...
                <section class="content-header">
                    <h1>
                        APPLICATION<small>View for Parent</small>
                    </h1>
                    <ol class="breadcrumb">
                                    ...
                                    <div class="row">
                                        <div class="col-md-12">
                                            ...
                                            <table id="table_view" class="table table-bordered table-striped table-condensed">
                                                ...
                                                <tbody>
                                                    <%
                                                        ArrayList<Object[]> registryData = RegistryController.getParentData(u);
                                                    ...
                                                    %>
                                                    <tr>
                                                        <td style="display: none" class="student-data"><%= data[39 - 1] + "%" + data[41 - 1] + "%" + data[12 - 1] + "%" + data[11 - 1] + "%" + data[15 - 1] + "%" + data[16 - 1] + "%" + data[17 - 1] + "%" + data[14 - 1] + "%" + data[33 - 1] + " " + data[32 - 1] + "%" + data[34 - 1] + "%" + data[23 - 1] + "%" + data[22 - 1] + "%" + data[35 - 1] + "%" + data[26 - 1] + "%" + data[25 - 1] + "%" + data[27 - 1] + "%" + data[5 - 1] + "%" + data[1]%></td>
                                                        <td><%= i + 1%></td>
                                                        <td><%= data[0]%></td>
                                                        <td><%= data[11]%></td>
                                                        <td><%= data[38]%></td>
                                                        ...
                                                        <td><a href="#"><button type="button" class="btn btn-xs btn-primary parent-view-application"><i class="fa fa-map-marker"></i> View Application</button></a></td>
                                                        <%
                                                            if (data[8].equals("passed")) {%>
                                                        <td><a href="#"><button type="button" class="btn btn-xs btn-success parent-active"><i class="fa fa-map-marker"></i> Active</button></a></td>
                                                        <td><a href="#"><button type="button" class="btn btn-xs btn-danger parent-inactive"><i class="fa fa-map-marker"></i> Inactive</button></a></td>
                                                        <% } else if (data[8].contains("failed") || data[8].contains("rejected") || data[8].contains("int-rejected")) {%>
                                                        <td><a href="#"><button type="button" class="btn btn-xs btn-success parent-view-reason"><i class="fa fa-map-marker"></i> View Reason</button></a></td>
                                            ...
                                            <p id="validityOfDate" style="display: none"><%= new Date().compareTo(parentdeadline_1.getP1()) <= 0 || (new Date().compareTo(gramadeadline_1.getG1()) >= 0 && new Date().compareTo(parentdeadline_2.getP2()) <= 0)%></p>
                                            <%}%>
                                        </div><!-- /.col -->
                                    </div>
                                </div>
        ...
        <script type="text/javascript">
            $('.parent-active').click(function () {
                ...
                $.ajax({
                    ...
                    data: {
                        student_id: data[17], register_id: register_id, function: 'parent-active'
                    },
                    success: function (responseText) {
                        if (responseText.indexOf("S") != -1) {
                            element.parent().parent().prev().prev().text("active");
                        } else {
                            alert('You have already selected a school!');
                        }
                    }
                });
            });
            $('.parent-inactive').click(function () {
                var element = $(this);
                var register_id = $(this).parent().parent().prev().prev().prev().prev().prev().prev().prev().prev().text();
                $.ajax({
                    type: "GET",
                    url: 'RegistryController',
                    data: {
                        register_id: register_id, status: 'inactive', function: 'parent-choice'
                    },
                    success: function (responseText) {
                        if (responseText.indexOf("{isSuccess:1}") != -1) {
                            element.parent().parent().prev().prev().prev().text("inactive");
                        }
                    }
                });
            });
            $('.parent-view-reason').click(function () {
                var reason = $(this).parent().parent().parent().children().last().prev().prev().text();
                $('#reasonModalButton').click();
                $('#reason').text(reason);
            });

            $('.parent-view-application').click(function () {
                var rawData = $(this).parent().parent().parent().children().first().text();
